...
export const axiosrequest=async({
    ...
    const headers=hasAuth? getAuthToken(): {}
        const response = await Axios({method,url,data:payload,headers:{...headers}}).catch(
            (e)=>{
                if(!showError) return
                notification.error({
...
export const getAllAvailableJobs=async(setAllAvailableJobsList, setFetching)=>{
        ...
        if(response){
            const data=response.data.Data.JobsList==null?[]:response.data.Data.JobsList
...
export const getPostedJobsByRecruiter=async(setPostedJobs,setFetching,username)=>{
        ...
        if(response){
            const data=response.data.Data.JobDetailsList==null?[]:response.data.Data.JobDetailsList
            setPostedJobs(data)
            setFetching(false)
        }
    }
...
export const getappliedApplicants=async(setAppliedApplicants,setFetching,username)=>{
        ...
        if(response){
            const data=response.data.Data.AppliedJoblist==null?[]:response.data.Data.AppliedJoblist
...
export const getAppliedJobs=async(setAppliedJobs,setFetching,username)=>{
        ...
        if(response){
            const data=response.data.Data.ApplieJobsByUserList==null?[]:response.data.Data.ApplieJobsByUserList
            setAppliedJobs(data)
            setFetching(false)
        }
    }